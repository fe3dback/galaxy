...
func TestRect2Point(t *testing.T) {
	...
	centered2x2 := engine.Rect{Min: engine.Vec{X: 0, Y: 0}, Max: engine.Vec{X: 2, Y: 2}}
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				a: centered2x2,
				b: engine.Vec{X: -0.9999, Y: -0.9999},
			},
			want: true,
		},
		{
		...
		// offset not collide
		{
			...
			args: args{
				a: engine.Rect{
					Min: engine.Vec{X: -1, Y: -1}, // x,y
					Max: engine.Vec{X: 2, Y: 2},   // w,h
				},
				b: engine.Vec{X: -2, Y: -2},
			},
			want: true,
		},
		{
			...
			args: args{
				a: engine.Rect{
					Min: engine.Vec{X: 0, Y: 0},             // x,y
					Max: engine.Vec{X: 1.99999, Y: 1.99999}, // w,h
				},
				b: engine.Vec{X: -2, Y: -2},
			},
			want: false,
...
func TestRect2Circle(t *testing.T) {
	...
	centeredCircle2R := engine.Circle{Pos: engine.Vec{}, Radius: 2}
	centeredBox2R := engine.Rect{
		Min: engine.Vec{},           // x,y
		Max: engine.Vec{X: 2, Y: 2}, // w,h
	}
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				r: centeredBox2R,
				c: engine.Circle{Pos: engine.Vec{X: 2, Y: 0}, Radius: 2},
			},
			want: true,
		},
		{
