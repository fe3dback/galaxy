...
import (
	...
	renderers2 "github.com/fe3dback/galaxy/internal/engine/lib/renderers"
...
func NewSDLLib(closer *utils2.Closer, defaultWidth, defaultHeight int, fullscreen bool) (*SDLLib, error) {
	platform := &SDLLib{}
	closer.EnqueueFree(platform.quit)

	defer utils2.CheckPanicWith("sdl lib", func() {
		platform.quit()
	})
	...
	utils2.Check("sdl init", err)

	err = ttf.Init()
	utils2.Check("ttf init", err)
	...
	if fullscreen {
		...
		utils2.Check("get display mode", displayModeErr)
	...
	utils2.Check("create window", err)
	...
	utils2.Check("failed to create OpenGL context", err)

	err = window.GLMakeCurrent(glContext)
	utils2.Check("failed to set current OpenGL context", err)

	_ = sdl.GLSetSwapInterval(1)

	// create engine renderer
	surface, err := window.GetSurface()
	utils2.Check("window get surface", err)

	err = surface.FillRect(nil, 0)
	utils2.Check("clear window surface", err)

	err = window.UpdateSurface()
	utils2.Check("update window surface", err)
	...
	utils2.Check("create engine renderer", err)
	...
	switch clientAPI {
	case SDLClientAPIOpenGL2:
		guiRenderer, err = renderers2.NewOpenGL2(platform.io)
	case SDLClientAPIOpenGL3:
		guiRenderer, err = renderers2.NewOpenGL3(platform.io)
	}

	utils2.Check("create GUI renderer", err)
